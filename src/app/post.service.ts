...
export class PostService {
    ...
    async getTopPost() {
        const indice = Math.floor(Math.random() * this.post.length);
        const selectedPost = this.post[indice];
        this.getRelated(4);
        return selectedPost;
    }
    getRelated(num: number): Post[] {
        const presenti: number[] = [];
        const related: Post[] = [];

        while (related.length < num) {
            const indice = Math.floor(Math.random() * this.post.length);
            if (!presenti.includes(indice)) {
                presenti.push(indice);
                related.push(this.post[indice]);
            }
        }
